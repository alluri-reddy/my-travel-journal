...
export default function Card({item, sendDataToParent}){
   ...
   const detail =(itemId)=>{
    ...
    return(
            <div className="card" onMouseOver={() => sendDataToParent(item.id)} >
                <div className="image" onClick={() => detail(item.id)} >
                ...
                <div className="inside-card-part2">
                        <div className="first-detail">
                            ...
                            <a className="location-link" href={item.locationlink}><p>View on Google Maps</p></a>
                        </div>
                        <div className="main-place-name"><h1>{item.mainplace}</h1></div>
                        <div className="date"><p>{item.dateofupload}</p></div>
                        <div className="each-place-info"><p>{item.discription}</p></div>
